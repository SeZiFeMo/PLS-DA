...
def plot_plot(x_values, y_values, cat=None):

    if cat is None:
        color = 'blue'
        linecolor = '#1F77B4'
    else:
        color = properties_of(cat)['face_color']
        linecolor = color
...
def scores_plot(model, pc_a, pc_b, matrix='x', normalize=False):
    ...
    for n in range(scores.shape[0]):
        ...
        scatter_plot(score_pc_a, score_pc_b, cat)
...
def data_plot(model):
    ...
    for i in range(model.dataset.shape[0]):
        ...
        plt.plot(range(model.dataset.shape[1]),
                model.dataset[i],
                color=properties_of(cat)['face_color'],         # line color
                linestyle='solid',
                alpha=.5,
                marker=properties_of(cat)['marker'],              # do not set it to
                markerfacecolor=properties_of(cat)['face_color'],  # marker color
                markeredgecolor=properties_of(cat)['edge_color'])
...
def modeled_Y_plot(model):
    ...
    for j in range(model.p):
        for i in range(model.n):
            cat = model.categories[i]
            scatter_plot(i, model.Y_modeled[i, j], cat)
